...
****************************
[Script]
Bili Region = type=http-response,pattern=^https:\/\/ap(p|i)\.bili(bili|api)\.(com|net)\/(pgc\/view\/v\d\/app\/season|x\/v\d\/search\/defaultwords)\?access_key,requires-body=1,max-size=0,script-path=https://raw.githubusercontent.com/ABreadTree/mineForStash/main/Bili_Auto_Regions.js
#可选, 适用于搜索指定地区的番剧
Bili Search = type=http-request,pattern=^https:\/\/ap(p|i)\.bili(bili|api)\.(com|net)\/x\/v\d\/search(\/type)?\?.+?%20(%E6%B8%AF|%E5%8F%B0|%E4%B8%AD)&,script-path=https://raw.githubusercontent.com/ABreadTree/mineForStash/main/Bili_Auto_Regions.js
[MITM]
...
****************************
[rewrite_local]
^https:\/\/ap(p|i)\.bili(bili|api)\.(com|net)\/(pgc\/view\/v\d\/app\/season|x\/v\d\/search\/defaultwords)\?access_key url script-response-body https://raw.githubusercontent.com/ABreadTree/mineForStash/main/Bili_Auto_Regions.js
#可选, 适用于搜索指定地区的番剧
^https:\/\/ap(p|i)\.bili(bili|api)\.(com|net)\/x\/v\d\/search(\/type)?\?.+?%20(%E6%B8%AF|%E5%8F%B0|%E4%B8%AD)& url script-request-header https://raw.githubusercontent.com/ABreadTree/mineForStash/main/Bili_Auto_Regions.js
[mitm]
hostname = ap?.bili*i.com, ap?.bili*i.net
[filter_local]
...
async function SwitchRegion(play) {
	...
	const TW = $.read('BiliArea_TW') || 'TW 台湾'; //Your Taiwan sub-policy name.
	const HK = $.read('BiliArea_HK') || 'TW 台湾'; //Your HongKong sub-policy name.
	const DF = $.read('BiliArea_DF') || 'TW 台湾'; //Sub-policy name used after region is blocked(e.g. url 404)
